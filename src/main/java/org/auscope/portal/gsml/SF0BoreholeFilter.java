...
public class SF0BoreholeFilter extends BoreholeFilter {
	...
	public String getFilterStringBoundingBox(FilterBoundingBox bbox) {
		...
		return this.generateFilter(this.generateAndComparisonFragment(this.generateBboxFragment(bbox, "gsmlp:shape"),
	...
	protected String generateFilterFragment() {
		...
		if (boreholeName != null && !boreholeName.isEmpty()) {
			parameterFragments.add(this.generatePropertyIsLikeFragment("gsmlp:name", "*" + this.boreholeName + "*"));
		}

		if (custodian != null && !custodian.isEmpty()) {
			parameterFragments
					.add(this.generatePropertyIsLikeFragment("gsmlp:boreholeMaterialCustodian", this.custodian));
		}

		if (dateOfDrilling != null && !dateOfDrilling.isEmpty()) {
			parameterFragments.add(this.generateOrComparisonFragment(
